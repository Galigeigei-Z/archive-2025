...
def ML4CE_uncon_table_plot(array, functions_test, algorithms_test, N_x_l, home_dir, timestamp, SafeFig = False):
    ...
    for i in range(num_tables):
        ...
        # 收集 DataFrame
        df = pd.DataFrame(matrix, columns=columns, index=rows_names_l)
        df.index.name = "Dimension"
        df["Algorithm"] = algorithms_test[i].__name__  # 增加算法列
        collected_dfs.append(df.reset_index())
        ...
        if SafeFig:
            ...
            # 保存 PNG
            plt.savefig(os.path.join(directory_name, f'{algorithms_test[i].__name__}.png'))
            plt.close()
        else:
    ...
    if SafeFig:
        ...
        all_df.to_csv(os.path.join(directory_name, "all_algorithms.csv"), index=False)
        print(f"All algorithm data saved to {os.path.join(directory_name, 'all_algorithms.csv')}")
...
def ML4CE_uncon_graph_abs(test_res, algs_test, funcs_test, N_x_l, home_dir, timestamp, SafeFig=False):
    ...
    plt.rcParams.update({
    "text.usetex": False,                 # 不用 LaTeX 引擎，直接系统字体
    "font.family": "Times New Roman",     # 设置 Times New Roman
    "font.weight": "bold",                # 全局粗体
    "axes.labelweight": "bold",           # 坐标轴标签粗体
    "axes.titlesize": 28,                 # 标题字号
    "axes.labelsize": 28,                 # 坐标轴标签字号
    "xtick.labelsize": 26,                # x 轴刻度字号
    "ytick.labelsize": 26,                # y 轴刻度字号
    "xtick.major.width": 2,               # x 轴刻度线加粗
    "ytick.major.width": 2,               # y 轴刻度线加粗
    "axes.linewidth": 2,                   # 坐标轴边框加粗
    "legend.fontsize": 24,          # legend 字号
    "legend.title_fontsize": 26,    # legend 标题字号（如果有的话）
    })
    ...
    for i_dim in range(len(N_x_l)):
        ...
        for i_fun in range(n_f):
            ...
            ax.text(
                0.97, 0.97, "(a)",
                transform=ax.transAxes,   # 坐标归一化到 [0,1]
                fontsize=28,
                fontweight="bold",
                family="Times New Roman",
                va="top", ha="right"      # 对齐方式：右上角
            )
            ...
            plt.legend(
                ...
                loc='lower left',  # 全部放在右上角
                frameon=True,       # 显示边框
                prop={'family': 'Times New Roman', 'weight': 'bold', 'size': 24},  # Times New Roman 粗体
            ).get_frame().set_linewidth(2.0)  # 边框加粗
            
        
            if SafeFig:
                # 确认目录
                directory_name = r"D:\lib\gali\Manuscript\table3\fig5-levy-noise0.01"
                if not os.path.isdir(directory_name):
                    os.makedirs(directory_name)

                # 保存 PNG 和 SVG
                plt.savefig(
                    os.path.join(directory_name, "{}_{}_1D.png".format(dim_S, funcs_test[i_fun])),
                    bbox_inches="tight", dpi=300
                )
                plt.savefig(
                    os.path.join(directory_name, "{}_{}_1D_20.svg".format(dim_S, funcs_test[i_fun])),
...
def ML4CE_uncon_contour_allin1(functions_test, algorithms_test, N_x_, x_shift_origin, bounds_, bounds_plot, SafeFig=False):
    ...
    for fun_ in functions_test:
        ...
        for alg_ in algorithms_test:
            ...
            # remove ticks
            # 设置刻度参数：主刻度朝里，小刻度不显示
            ax3.tick_params(axis='both', direction='in', which='major', length=6, width=1.5)
            ax3.tick_params(axis='both', which='minor', bottom=False, left=False)
        ...
        if SafeFig == True:
            ...
            def directory_exists(directory_name):
                root_directory = os.getcwd()  # Root directory
            ...
            if directory_exists(directory_name):
                ...
                if 'LSQM' in legend_cust:
                    plt.savefig(directory_name + '/{}_allin1_A.svg'.format(f_feas.func_type),
                                format='svg', bbox_inches='tight')
                else:
                    plt.savefig(directory_name + '/{}_allin1_B.svg'.format(f_feas.func_type),
                                format='svg', bbox_inches='tight')
            else:
...
def ML4CE_uncon_contour_allin1_smooth(functions_test, algorithms_test, N_x_, x_shift_origin, reps, bounds_, bounds_plot, SafeFig=False):
    ...
    "font.weight": "bold",                # 全局粗体
    "axes.labelweight": "bold",           # 坐标轴标签粗体
    "axes.titlesize": 32,                 # 标题字号
    "axes.labelsize": 34,                 # 坐标轴标签字号
    "xtick.labelsize": 28,                # x 轴刻度字号
    "ytick.labelsize": 28,                # y 轴刻度字号
    "xtick.major.width": 2,               # x 轴刻度线加粗
    "ytick.major.width": 2,               # y 轴刻度线加粗
    "axes.linewidth": 2,                   # 坐标轴边框加粗
    "legend.fontsize": 27,                 # legend 字号
    "legend.title_fontsize": 26,    # legend 标题字号（如果有的话）
    })
    ...
    for fun_ in functions_test:
        ...
        # Plot the contour with normalization
        contour = plt.contourf(X1, X2, y, levels=200, cmap='RdYlBu_r')
        ...
        for alg_ in algorithms_test:
            ...
            runtime_results[alg_.__name__] = elapsed_time
            ...
            ax3.plot(best_values_x1[-1], best_values_x2[-1], colors[alg_index], marker='H',markersize=15, zorder=10)
            # highlight end point
            x_end, y_end = best_values_x1[-1], best_values_x2[-1]
            ax3.plot(x_end, y_end, colors[alg_index], marker='H', markersize=15, zorder=10)
            x_end, y_end = float(x_end), float(y_end)   # 确保是 float
            # 计算 (1,1) 的欧氏距离并打印
            dist = np.sqrt((x_end - 1.0) ** 2 + (y_end - 1.0) ** 2)
            print(f"{alg_.__name__}: Final point ({x_end:.4f}, {y_end:.4f}), Distance to (1,1) = {dist:.4f}")
            ax3.text(
                0.97, 0.97, "(d)",
                transform=ax3.transAxes,   # 坐标归一化到 [0,1]
                fontsize=32,
                fontweight="bold",
                family="Times New Roman",
                va="top", ha="right"      # 对齐方式：右上角
        ...
        legend_cust_dict = {
            ...
            'opt_SnobFit':'SNOBFIT',
            'opt_COBYLA':'COBYLA',
            'opt_CUATRO':'opt_CUATRO',
            'BO_np_scipy':'BO',
            'opt_ENTMOOT':'ENTMOOT',
            'NDCT_standard_PSO':'NDCT-standard-PSO',
            'NDCT_modified_PSO':'NDCT-modified-PSO'
        }

        legend_cust = []
        for key in algorithms_test:
            # 如果是字符串，直接用；如果是函数或类，用 __name__
            alg_name = key if isinstance(key, str) else key.__name__
            # 优先用映射字典，否则回退用原始名字
            legend_cust.append(legend_cust_dict.get(alg_name, alg_name))
    # ------- 输出运行时间 ------- #
    print("\nAlgorithm runtime summary:")
    for alg_name, t in runtime_results.items():
        print(f"{alg_name}: {t:.2f} seconds (for {reps} runs)")
    ...
    # Add start/end handles
    start_handle = Line2D([0], [0], marker='X', color='black', linestyle='None',
                          markersize=15, label='Start')
    end_handle = Line2D([0], [0], marker='H', color='black', linestyle='None',
    ...
    # ===== 保存完整图 =====
    plt.tight_layout()
    plt.savefig(r"D:\lib\gali\Manuscript\fig5-levy\d.png",
