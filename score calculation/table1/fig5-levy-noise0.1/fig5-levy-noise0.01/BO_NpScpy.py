...
class GP_model:
    ...
    def __init__(self, X, Y, kernel, multi_hyper, var_out=True):
        ...
        # 如果有数值非常接近的点，也删掉
        tol = 1e-8
        _, idx_tol = np.unique(np.round(X_unique, decimals=8), axis=0, return_index=True)
        X_unique = X_unique[idx_tol]
        Y_unique = Y_unique[idx_tol]
    ...
    def negative_loglikelihood(self, hyper, X, Y):
        ...
        K = self.Cov_mat(kernel, X, W, sf2)          # (n x n) 无噪声核
        K = (K + K.T) * 0.5                          # 数值对称化
        K += (sn2 + 1e-8) * np.eye(n_point)          # 噪声 + 基础 jitter

        # 进一步的稳健策略：尝试分解，不行就逐步增加 jitter
        jitter = 1e-8
        for _ in range(5):
            try:
                ...
                jitter *= 10.0
                K += jitter * np.eye(n_point)
        else:
            raise np.linalg.LinAlgError("Covariance not PD even after adding jitter.")
...
def BO(f, x_dim, bounds, iter_tot, has_x0=False):
    ...
    if has_x0:
        # 固定起点 [-3.5, 4.0]，必须是 (1,2) 二维数组
        x0 = np.array([[-3.5, 4.0]], dtype=float)
        d_ = ['data0', x0, 10]   # 'data0' 模式: 提供起点+初始样本数
