...
def ENTMOOT(t_, N_x_, bounds_, f_eval_, has_x0 = False):
    ...
    if has_x0 == True:
        ...
        x_start = np.array([-3.5, 4.0], dtype=float)  # shape = (2,)
        ...
        train_x = np.insert(train_x_rand, 0, x_start, axis=0).transpose()
        train_y = np.insert(train_y_rand, 0, y_start, axis=0)
